class Event < ActiveRecord::Base
  ...
  named_scope :next_10, :conditions => ['start_date >= ? OR end_date >= ?', Date.today, Date.today], :limit => 10 
  ...
  named_scope :to_come, :conditions => ['start_date >= ? OR end_date >= ?', Date.today, Date.today]
